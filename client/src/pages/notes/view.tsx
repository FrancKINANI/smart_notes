...
import { enhanceNote, generateQuiz, generateFlashcards, textToSpeech } from "@/lib/openai";
...
export default function ViewNote() {
  ...
  // Fetch note details
  const { data: note, isLoading: isLoadingNote, error } = useQuery({
  ...
  const handlePlayback = () => {
    if (isSpeaking) {
      ...
      const textToRead = activeTab === "enhanced" && note.enhancedContent 
      ...
      <div className="py-6">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <Alert variant="destructive">
            <AlertDescription>
              Failed to load note. It may have been deleted or you don't have permission to view it.
            </AlertDescription>
          </Alert>
          <div className="mt-4">
            <Button asChild>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="enhanced">
            {note?.enhancedContent ? (
              <div className="prose max-w-none">
                {note.enhancedContent.split('\n').map((paragraph: string, i: number) => (
                  <p key={i}>{paragraph}</p>
                ))}
              </div>
            ) : (
              <div className="text-center py-12">
                <p className="text-gray-500">No enhanced content available yet. Click "Enhance with AI" to generate it.</p>
              </div>
            )}
          </TabsContent>
